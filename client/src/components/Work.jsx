...
const Work = ({isSidebarOpen, onEditTask, currentTask, setCurrentTask, showModal, setShowModal}) => {
  ...
  console.log('workTasks: ', workTasks);
    ...
    return (
        ...
        <MDBTypography tag='span' className="fw-bold display-6 py-5">
                Work <MDBTypography tag='span' className="float-end ms-auto border bg-secondary bg-opacity-25 px-2 text-warning rounded">{workTasks.length}</MDBTypography>
            </MDBTypography>
        ...
        <MDBListGroup light style={{ minWidth: '22rem' }}>
          {workTasks.length >= 1 ? workTasks.map(item => (
        ...
        <MDBBtn onClick={() => handleGetSingleTask(item.id)} className={`${checkedItems[item.id] ? 'd-none' : ''} me-1 rounded-pill bg-transparent border border-secondary`}><MDBIcon fas icon="eye" color="secondary" size="lg" /></MDBBtn>
        <MDBBtn onClick={() => handleDelete(item.id)} className="me-1 rounded-pill bg-transparent border border-danger">
          <MDBIcon fas icon='trash' color='danger' size='lg'/>
        </MDBBtn>
          </MDBListGroupItem>
          )) : <MDBListGroupItem className="text-center fw-bold text-warning bg-secondary bg-opacity-25 mt-5">No Work task(s)</MDBListGroupItem>}
