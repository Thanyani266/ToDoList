        ...
        <MDBRow>
          {notes.length >= 1 ? (
            notes.map((item) => {
              return (
                <MDBCol lg="4" md="6" className="mb-3" key={item.id}>
                  <MDBCard
                    style={{ height: '300px' }}
                    className={`${item.background} bg-opacity-25 pe-auto`}
                  >
                    <MDBCardBody className="d-flex flex-column">
                      <MDBCardTitle>{TrimTitle(item.title)}</MDBCardTitle>
                      <MDBCardText>{TrimDesc(item.description)}</MDBCardText>
                      <div className="mt-auto">
                        <MDBBtn className="btn-outline-dark p-0 fs-4 px-1">
                          <MDBIcon
                            fas
                            icon="eye"
                            onClick={() => handleGetSingleNote(item.id)}
                          />
                        </MDBBtn>
                      </div>
                    </MDBCardBody>
            ...
            <>
              <MDBTypography
                tag="div"
                className="text-center fw-bold text-warning bg-secondary bg-opacity-25 mt-5 py-5"
              >
                No note(s)
              </MDBTypography>
              <MDBBtn
              ...
              >
                <MDBIcon fas icon="plus" className="me-2" /> new note
              </MDBBtn>
            </>
          )}
          ...
          <ModalOne show={showModalOne} onClose={handleCloseModalOne}>
            {selectedNote ? (
              ...
              >
                ...
                <div className="fs4 border bg-secondary bg-opacity-25 p-2 rounded mb-2">
                  <span className="fw-bold text-warning">Title: </span>
          ...
          {notes.length >= 1 ? (
            <MDBCol lg="4" md="6" className="mb-3 my-auto">
              <MDBCard
                className="bg-secondary text-center text-light p-5"
                onClick={handleOpenModal}
