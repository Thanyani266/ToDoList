  ...
  return (
    <MDBCol className={`${isSidebarOpen ? 'content-shifted' : 'content'}`}>
      <MDBTypography tag="span" className="fw-bold display-6 py-5">
        ...
        >
          {todayTasks.length + tomorrowTasks.length + weekendTasks.length}
        </MDBTypography>
      </MDBTypography>
      ...
      <MDBBtn
        className="w-100 text-start mt-2 bg-transparent border text-success"
            ...
            >
              <>
                ...
                <MDBBtn
                  ...
                  className={`${checkedItems[item.id] ? 'd-none' : ''} me-1 rounded-pill btn-outline-info`}
                >
                  <MDBIcon fas icon="edit" size="lg" />
                </MDBBtn>
                <MDBBtn
                  onClick={() => handleGetSingleTask(item.id)}
                  className={`${checkedItems[item.id] ? 'd-none' : ''} me-1 rounded-pill btn-outline-secondary`}
                >
      ...
      <MDBRow className="mb-3">
        ...
        <MDBCol md="6">
          ...
          <MDBListGroup light style={{ minWidth: '22rem' }}>
            {weekendTasks.length >= 1 ? (
              weekendTasks.map((item) => (
                <MDBListGroupItem
                  className={`d-flex justify-content-between align-items-start border border-2 rounded mt-2 ${checkedItems[item.id] ? 'text-light bg-secondary bg-opacity-25' : ''}`}
                  key={item.id}
                >
