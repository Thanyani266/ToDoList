...
const Personal = ({
  ...
  return (
    <MDBCol className={`${isSidebarOpen ? 'content-shifted' : 'content'}`}>
      ...
      >
        <MDBIcon fas icon="plus" className="me-2" /> new task{' '}
      </MDBBtn>
      <MDBListGroup light style={{ minWidth: '22rem' }}>
        {personalTasks.length >= 1 ? (
          personalTasks.map((item) => (
            <MDBListGroupItem
              className={`d-flex justify-content-between align-items-start rounded border border-2 mt-2 ${checkedItems[item.id] ? 'text-light bg-secondary bg-opacity-25' : ''}`}
              key={item.id}
            >
